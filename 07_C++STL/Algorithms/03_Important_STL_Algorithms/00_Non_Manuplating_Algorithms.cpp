...
int main() {
    ...
    (binary_search(arr, arr+n, 20)) 
        ? cout << "\nElement: 20 exists in the vector."
        : cout << "\nElement: 20 does not exist in the vector.\n";
...
8. find(first_iterator, last_iterator, x) –> Returns an iterator to the first occurrence of x in vector 
...
10. lower_bound(first_iterator, last_iterator, x) –> returns an iterator pointing to the first element in the range [first,last) which 
has a value not less than ‘x’.

11. upper_bound(first_iterator, last_iterator, x) –> returns an iterator pointing to the first element in the range [first,last)
